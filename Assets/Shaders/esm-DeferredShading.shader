        ...
        Pass
        {
            ...
            float pcf5x5(float4 lightScreenPos, float d)
            {
                ...
                for (int i = -2; i <= 2; i++)
                {
                    ...
                    {
                        float z = 1 - tex2Dlod(
                            smTex, float4(lightScreenPos.xy + 1 * float2(j, i) / shadowmapSize, 0, 0)).r;
                        shadow += d - 0.003 < z;
                    }
                }
            ...
            half4 CalculateLight(unity_v2f_deferred i)
            {
                ...
                half4 res = UNITY_BRDF_PBS(data.diffuseColor, data.specularColor, oneMinusReflectivity, data.smoothness,
                                           data.normalWorld, -eyeVec, light, ind);
